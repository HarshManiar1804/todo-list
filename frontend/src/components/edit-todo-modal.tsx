...
export default function EditTodoModal({
  ...
  const [assignedUsers, setAssignedUsers] = useState<string[]>(
    todo.assignedUsers?.map((user: any) => user._id) || []
  );
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setTags(todo.tags || []);
      setAssignedUsers(todo.assignedUsers?.map((user: any) => user._id) || []);
      setTagInput("");
    }
  }, [isOpen, todo]);
  ...
  const handleSubmit = () => {
    ...
    console.log("Submitting updatedTodo:", updatedTodo);
          ...
          <div className="grid gap-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              {users.map((user) => (
                <div
                  key={user._id}
                  className={`p-2 border rounded-md cursor-pointer flex items-center gap-2 ${assignedUsers.includes(user._id)
